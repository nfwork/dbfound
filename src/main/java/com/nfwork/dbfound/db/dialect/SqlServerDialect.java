...
public class SqlServerDialect implements SqlDialect {
	...
	public String getPagerSql(String sql, int pagerSize, long startWith) {
		...
		eSql = eSql.substring(0, selectIndex + 6) + " row_number() over(order by " + orderBy + ") _drm ," + eSql.substring(selectIndex + 6);

		return " select * from (" + eSql + ") v where _drm <=" + (startWith + pagerSize) + " and _drm >= " + (startWith + 1);
	}
